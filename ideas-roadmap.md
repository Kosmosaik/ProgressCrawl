1. Rarity animation: When the loot slider/progress is finished, if the item is higher than Common (Uncommon, Rare, Exotic etc), animate the slider with a pop up like effect,
...
2. Sort items by category (header, lines etc). Add category to hover tooltip.
3. Change quality drop weight based on tier letter AND tier number (F9 most common, S1 rarest).
...
FEATURE: Character & Classes
  @ Add character system with stats (Name, Class, Stats (Strength, Vitality, Agility - HP, Encumbrance, Attack Speed, Evade)).
  @ When game starts - Show character screen (hide loot button etc).
